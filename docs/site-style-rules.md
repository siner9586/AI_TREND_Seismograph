...
最新热点卡片必须采用“确定性自然语言总结”，不调用大模型。总结器以结构化字段为依据，核心字段包括：
...
- `curated_summary`：作为“抓取目标全文总结”。必须使用完整自然语言段落，说明本次围绕哪个方向抓取、抓到了哪些代表性目标、这些目标集中说明什么、工程侧/研究侧/机构账号侧各有什么线索，以及当前判断的边界。该字段不是简单罗列数字，而是面向普通读者的可读解读。
- `signal_takeaways`：作为“总体归纳结论”。必须归纳多个抓取目标的总体意义，覆盖总体判断、内容重点、工程侧归纳、研究侧归纳、机构账号归纳、证据链归纳和后续观察建议。
- `source_link_groups`：按“代表项目、论文来源、方法与数据集信号、其他信号”分组的来源链接。
- `evidence_digest`：对代表性证据给出“为什么纳入”的短说明。
- `signal_focus`：用于前端解释的主导来源、核心方法、代表目标、机构线索和计数字段。
...
1. 主题、标签与震级。
2. 原始摘要 `summary`。
3. “抓取目标全文总结”：优先显示 `curated_summary`，并注明“确定性规则总结 · 不调用大模型”。
4. “总体归纳结论”：显示 `signal_takeaways` 或 fallback 结论。
